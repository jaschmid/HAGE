...
namespace HAGE {
	...
	"  float3 normal = VS_OUT.normal.xyz;\n"
	...
	"    if(Odist[i] <= Sdist[i])\n"
	"    {\n"
	"      float3 normDir = lDir[i]/length(lDir[i]);\n"
	"      float light_intensity = dot(normDir,normal);\n"
	"	   light_intensity = clamp(light_intensity,0.0f,1.0f);\n"
	"      PS_OUT.color += float4(light_intensity*VS_OUT.color.xyz*LightColor[i].xyz,0.0f);\n"
	"    }\n"
	"  PS_OUT.color.x = clamp(PS_OUT.color.x,0.0f,1.0f);\n"
	"  PS_OUT.color.y = clamp(PS_OUT.color.y,0.0f,1.0f);\n"
	"  PS_OUT.color.z = clamp(PS_OUT.color.z,0.0f,1.0f);\n"
	"}\n";
