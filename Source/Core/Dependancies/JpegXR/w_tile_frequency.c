                         ...
                         unsigned tx, unsigned ty)
{
	...
	const unsigned int tempBuffer_size = 128*1024;
	unsigned char tempBuffer[tempBuffer_size];

    _jxr_wbitstream_initialize(&strFP, tempBuffer,tempBuffer_size);
    ...
    if (bands_present == 0 /* ALL */) {
        ...
        _jxr_rbitstream_initialize(&strFPRead, tempBuffer,tempBuffer_size);
