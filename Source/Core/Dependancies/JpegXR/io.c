...
void _jxr_rbitstream_initialize(struct rbitstream*str, const unsigned char* buffer, unsigned int buffer_size)
{
    str->bits_avail = 0;
	str->buffer_pos = 0;
	str->buffer = buffer;
...
void _jxr_rbitstream_mark(struct rbitstream*str)
{
	...
	str->mark_stream_position = str->buffer_pos;
...
void _jxr_rbitstream_seek(struct rbitstream*str, uint64_t off)
{
	...
	str->buffer_pos = str->mark_stream_position + (long)off;
...
static int get_byte(struct rbitstream*str)
{
    int tmp;
    assert(str->bits_avail == 0);
	if( str->buffer_pos >= str->buffer_size )
		return EOF;
	else
		tmp = str->buffer[str->buffer_pos++];
...
void _jxr_wbitstream_initialize(struct wbitstream*str, unsigned char* buffer, unsigned int buffer_size)
{
    str->byte = 0;
    str->bits_ready = 0;
	str->buffer_pos = 0;
...
static void put_byte(struct wbitstream*str)
{
	...
	assert( str->buffer_pos <= str->buffer_size);
	str->buffer[str->buffer_pos++] = str->byte;
    str->byte = 0;
    str->bits_ready = 0;
    str->write_count += 1;
}
