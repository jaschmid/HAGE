...
int jxr_write_image_bitstream(jxr_image_t image, unsigned char* buffer, unsigned int buffer_size)
{
    int rc, res = 0;

    struct wbitstream bits;
    _jxr_wbitstream_initialize(&bits, buffer,buffer_size);
    ...
    if (INDEXTABLE_PRESENT_FLAG(image)) {
		...
		const unsigned int codedBuffer_size= 1024*32;
		unsigned char codedBuffer[codedBuffer_size];

        struct wbitstream strCodedTiles;
        _jxr_wbitstream_initialize(&strCodedTiles, codedBuffer,codedBuffer_size);
        ...
        _jxr_rbitstream_initialize(&strCodedTilesRead, codedBuffer,codedBuffer_size);
