...
static const char* preproc = 
	...
	"		#define H_SAMPLE_2D_CMP(sampler,coord,cmp) (texture(sampler,float3((coord).x,(coord).y,(cmp))))\n"
	"		#define H_SAMPLE_CUBE_CMP(sampler,coord,cmp) (texture(sampler,float4((coord).x,(coord).y,(coord).z,(cmp))))\n"
	"		#define H_SAMPLE_2D_CMP0(sampler,coord,cmp) (textureLod(sampler,float3((coord).x,(coord).y,(cmp)),0.0f))\n"
